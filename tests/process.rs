...
#[test]
fn process_primitives_under_libc_so() {
    ...
    let src = fixtures.join("process_test.c");
    let bin = fixtures.join("process_test");
    let status = Command::new("musl-gcc")
        ...
        .status()
        .expect("failed to run musl-gcc for process_test");
    assert!(status.success(), "musl-gcc process_test compilation failed");

    let output = Command::new(&bin)
        .env("LD_LIBRARY_PATH", manifest_dir.join("target/debug").to_str().unwrap())
        .output()
        .expect("failed to run process_test");

    assert!(
        output.status.success(),
        "process_test exited with {:?}, stderr: {}",
        output.status.code(),
